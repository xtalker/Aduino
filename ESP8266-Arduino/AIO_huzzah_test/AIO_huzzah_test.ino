...
#define LAMP            0  // power switch tail

/************************* WiFi Access Point *********************************/

#define WLAN_SSID       "6fusion"
#define WLAN_PASS       "W4C1TG00D"
...
const char LAMP_FEED[] PROGMEM = AIO_USERNAME "/feeds/" FEED;
...
void loop() {
  ...
  if(! mqtt.ping(3)) {
    ...
    if(! mqtt.connected()) {
      Serial.println("No ping, reconnecting");
      connect();
    }
  }
  ...
  while (subscription = mqtt.readSubscription(1000)) {
    ...
    if (subscription == &lamp) {
      ...
      // Look for "ON" in the char array (its not a string)
      if ( (value[0] == 'O') && (value[1] == 'N') ) {
        // Note: Huzzah LED seems to be reverse logic!
